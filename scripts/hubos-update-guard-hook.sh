#!/usr/bin/env bash
set -euo pipefail
# HubOS Update Guard Hook — Called by hubos-auto-update before applying updates
#
...
GUARD="/usr/bin/hubos-update-guard"

# Fallback to local path if not installed system-wide
if [[ ! -x "$GUARD" ]]; then
    GUARD="$(dirname "$(readlink -f "$0")")/hubos-update-guard"
fi

if [[ ! -x "$GUARD" ]]; then
    echo "[update-guard-hook] hubos-update-guard not found — skipping pre-update checks"
    exit 0
fi
...
UNIT_NAME="hubos-update-guard-postboot"
...
[Unit]
Description=HubOS Update Guard — Post-Update Health Check
...
EOF
    ...
    echo "[update-guard-hook] systemd-run not available — run 'hubos-update-guard post-update' manually after reboot."
fi
