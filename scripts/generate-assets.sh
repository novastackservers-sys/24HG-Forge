#!/bin/bash
# HubOS Asset Generator — MUST run before building the OCI image
...
SVG="${ROOT}/branding/icons/hubos-logo.svg"
if [ ! -f "$SVG" ]; then
    echo "ERROR: Logo SVG not found at ${SVG}"
    exit 1
fi

echo "=== HubOS Asset Generator ==="
echo ""
...
for size in 16 24 32 48 64 128 256 512; do
    rsvg-convert -w "$size" -h "$size" "$SVG" -o "${ICON_DIR}/hubos-logo-${size}.png"
done
...
for size in 16 24 32 48 64 128 256 512; do
    cp "${ICON_DIR}/hubos-logo-${size}.png" "${ELECTRON_ICONS}/${size}x${size}.png"
done
echo "  Copied to hub-app/assets/icons/"

# ─── 3. Plymouth boot splash ───
echo "[3/8] Generating Plymouth assets..."
PLYMOUTH_DIR="${ROOT}/system_files/usr/share/plymouth/themes/hubos"
mkdir -p "$PLYMOUTH_DIR"
cp "${ICON_DIR}/hubos-logo-128.png" "${PLYMOUTH_DIR}/logo.png"
...
# Logo for GRUB (128px)
cp "${ICON_DIR}/hubos-logo-128.png" "${GRUB_DIR}/logo.png"
...
SDDM_DIR="${ROOT}/system_files/usr/share/sddm/themes/hubos"
mkdir -p "$SDDM_DIR"
cp "${ICON_DIR}/hubos-logo-128.png" "${SDDM_DIR}/logo.png"

# Preview thumbnail (400x300)
convert -size 400x300 xc:"#0a0a14" \
    \( "${ICON_DIR}/hubos-logo-64.png" -gravity center \) -composite \
    -fill "#ffffff" -font "DejaVu-Sans-Bold" -pointsize 18 \
    -gravity south -annotate +0+40 "HubOS" \
...
cp "${ICON_DIR}/hubos-logo-256.png" "${INSTALLER_DIR}/logo.png"

# Welcome image (wider, with text)
convert -size 600x400 xc:"#0a0a14" \
    \( "${ICON_DIR}/hubos-logo-256.png" -gravity center \) -composite \
...
convert -size 3840x2160 \
    ...
    -blur 0x40 \
    \( "${ICON_DIR}/hubos-logo-512.png" -channel A -evaluate Multiply 0.08 +channel \
       -gravity center \) -composite \
    "${WALL_DIR}/hubos-dark.png"
...
convert -size 3840x2160 \
    ...
    \( "${ICON_DIR}/hubos-logo-512.png" -channel A -evaluate Multiply 0.06 +channel \
       -gravity center \) -composite \
    "${WALL_DIR}/hubos-neon.png"
...
convert -size 3840x2160 xc:"#060610" \
    ...
    -draw "$CIRCUIT_DRAWS" \
    \( "${ICON_DIR}/hubos-logo-512.png" -channel A -evaluate Multiply 0.05 +channel \
       -gravity center \) -composite \
    "${WALL_DIR}/hubos-circuit.png"

echo "  Created 4 wallpapers (3840x2160)"

# ─── 8. Pixmap for system use ───
echo "[8/8] Installing system pixmap..."
cp "${ICON_DIR}/hubos-logo-256.png" "${ROOT}/branding/icons/hubos-logo.png"
