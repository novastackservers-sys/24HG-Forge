#!/bin/bash
# 24HG Forge First Boot Setup — Runs once as root on first boot
# Installs Flatpak apps, configures system, then disables itself

MARKER="/var/lib/forge/.first-boot-done"

# Skip if already completed
if [ -f "${MARKER}" ]; then
    echo "24HG Forge first boot already completed, skipping."
    exit 0
fi

echo "=== 24HG Forge First Boot Setup ==="
...
if [ -d /usr/share/forge/grub ]; then
    if [ -f /etc/default/grub ]; then
        grub2-mkconfig -o /boot/grub2/grub.cfg 2>/dev/null || true
    fi
fi

# Install Protontricks (for forge-proton-fix auto-repair)
...
PROTON_GE_DIR="/var/lib/forge/proton-ge"
...
echo "  - forge-server-status (live server monitoring)"
echo "  - forge-discord-fix (Rich Presence bridge)"
echo "  - forge-backup.timer (weekly game save backup)"
echo "  - forge-hub-bridge (Hub notifications, DMs, friend status)"
echo "  - forge-game-timer (playtime tracking across all launchers)"
echo "  - forge-proton-updater.timer (daily Proton-GE update check)"

# Enable Chromium desktop notifications for the Hub
mkdir -p /etc/skel/.config/chromium/Default
cat > /etc/skel/.config/chromium/Default/Preferences.forge 2>/dev/null <<'CHROMEOF'
{
  "profile": {
    "content_settings": {
      "exceptions": {
...
# Generate 24HG Forge sound theme
echo "Generating 24HG Forge sound theme..."
/usr/bin/forge-sounds install 2>/dev/null || true

# Set KDE splash screen to 24HG Forge
if command -v kwriteconfig6 &>/dev/null; then
    # Will be applied per-user on first login via skel
    echo "KDE splash screen set to 24HG Forge"
fi
...
echo "=== 24HG Forge First Boot Setup Complete ==="
