#!/usr/bin/env bash
set -euo pipefail
# 24HG Forge Update Guard Hook — Called by forge-auto-update before applying updates
#
...
GUARD="/usr/bin/forge-update-guard"

# Fallback to local path if not installed system-wide
if [[ ! -x "$GUARD" ]]; then
    GUARD="$(dirname "$(readlink -f "$0")")/forge-update-guard"
fi

if [[ ! -x "$GUARD" ]]; then
    echo "[update-guard-hook] forge-update-guard not found — skipping pre-update checks"
    exit 0
fi
...
UNIT_NAME="forge-update-guard-postboot"
...
[Unit]
Description=24HG Forge Update Guard — Post-Update Health Check
...
EOF
    ...
    echo "[update-guard-hook] systemd-run not available — run 'forge-update-guard post-update' manually after reboot."
fi
