#!/bin/bash
# 24HG Heartbeat — Reports system info to the 24HG Control Room
# Runs on boot and periodically via systemd timer.

set -euo pipefail

LOG_TAG="24hg-heartbeat"
API_URL="https://os.24hgaming.com/api/heartbeat"
VERSION_FILE="/usr/share/24hg/version"
...
if [ "$VERSION" = "unknown" ] && grep -q "HG24_VERSION" /usr/lib/os-release 2>/dev/null; then
    VERSION=$(grep "HG24_VERSION" /usr/lib/os-release | cut -d= -f2 | tr -d '"')
fi
