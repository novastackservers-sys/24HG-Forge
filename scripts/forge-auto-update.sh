#!/bin/bash
# 24HG Forge Auto-Update — Checks for and stages OS updates
# Runs daily via systemd timer. Does NOT reboot automatically.

set -euo pipefail

LOG_TAG="forge-update"
...
# Check for updates
log "Checking for 24HG Forge updates..."
        ...
        if [ -d "$RUNTIME_DIR" ]; then
            sudo -u "$USER_NAME" \
                ...
                notify-send \
                    ...
                    --icon=forge-logo \
                    "System Update Ready" \
                    "A 24HG Forge update has been downloaded. Reboot to apply." \
                    2>/dev/null || true
        fi
    done
else
