#!/bin/bash
# 24HG First Boot Setup — Runs once as root on first boot
# Installs Flatpak apps, configures system, then disables itself

MARKER="/var/lib/24hg/.first-boot-done"

# Skip if already completed
if [ -f "${MARKER}" ]; then
    echo "24HG first boot already completed, skipping."
    exit 0
fi

echo "=== 24HG First Boot Setup ==="
...
if [ -d /usr/share/24hg/grub ]; then
    if [ -f /etc/default/grub ]; then
        grub2-mkconfig -o /boot/grub2/grub.cfg 2>/dev/null || true
    fi
fi

# Install Protontricks (for 24hg-proton-fix auto-repair)
...
PROTON_GE_DIR="/var/lib/24hg/proton-ge"
...
echo "  - 24hg-server-status (live server monitoring)"
echo "  - 24hg-discord-fix (Rich Presence bridge)"
echo "  - 24hg-backup.timer (weekly game save backup)"
echo "  - 24hg-hub-bridge (Hub notifications, DMs, friend status)"
echo "  - 24hg-game-timer (playtime tracking across all launchers)"
echo "  - 24hg-proton-updater.timer (daily Proton-GE update check)"

# Enable Chromium desktop notifications for the Hub
mkdir -p /etc/skel/.config/chromium/Default
cat > /etc/skel/.config/chromium/Default/Preferences.24hg 2>/dev/null <<'CHROMEOF'
{
  "profile": {
    "content_settings": {
      "exceptions": {
...
# Generate 24HG sound theme
echo "Generating 24HG sound theme..."
/usr/bin/24hg-sounds install 2>/dev/null || true

# Set KDE splash screen to 24HG
if command -v kwriteconfig6 &>/dev/null; then
    # Will be applied per-user on first login via skel
    echo "KDE splash screen set to 24HG"
fi

# Mark completion
mkdir -p /var/lib/24hg
touch "${MARKER}"

echo "=== 24HG First Boot Setup Complete ==="
