...
HubOS ships with pre-configured keyboard shortcuts for gaming, screenshots, recording, and system control. These are set during installation and work out of the box.

## HubOS Custom Shortcuts

These are unique to HubOS and are configured in `~/.config/kglobalshortcutsrc.d/hubos.conf`:

| Shortcut | Action | Tool |
|----------|--------|------|
| `Print` | Full-screen screenshot | `hubos-screenshot full` |
| `Shift+Print` | Select area screenshot | `hubos-screenshot area` |
| `Meta+Print` | Active window screenshot | `hubos-screenshot window` |
| `F9` | Save instant replay clip | `hubos-replay save` |
| `Meta+N` | Toggle night light | `hubos-nightlight toggle` |
...
If you use `hubos-stream` with OBS Studio, these are the default scene/recording shortcuts:
...
GameMode does not have keyboard shortcuts -- it activates automatically when a game launches (via `hubos-smart-launch` daemon) and deactivates when the game closes. You can check its status:
...
3. Find HubOS shortcuts under **Custom Shortcuts**.
4. Click any shortcut to rebind it.

### Via Config File

Edit `~/.config/kglobalshortcutsrc.d/hubos.conf` directly:

```ini
[hubos-replay-save]
Comment=Save Instant Replay Clip
Exec=/usr/bin/hubos-replay save
Name=Save Replay
Shortcut=F9
```
...
- **Screenshot location:** Screenshots are saved to `~/Pictures/HubOS Screenshots/` by default.
- **Replay clips:** Replay clips are saved to `~/Videos/HubOS Clips/`.
