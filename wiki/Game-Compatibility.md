...
24HG Forge runs on Linux, which means most games run through a compatibility layer called **Proton** (Valve's fork of Wine). This page explains how game compatibility works and what tools 24HG Forge provides to check and improve it.
...
forge-compat check 730              # CS2

# Check a game by name
forge-compat check "Elden Ring"

# Search for a game
forge-compat search "Cyberpunk"
...
forge-anticheat-tracker scan

# Check a specific game
forge-anticheat-tracker check "PUBG"

# Show recent status changes (games that added/removed Linux support)
forge-anticheat-tracker updates

# Show the anti-cheat database
forge-anticheat-tracker database
```

The tracker runs as a background timer (`forge-anticheat-tracker.timer`) and sends a desktop notification whenever a game in your library changes anti-cheat status. For example, if Fortnite enables EAC on Linux, you will get a notification the same day.
...
Common issues:
- **Stuttering on first launch:** Shader compilation. Run `forge-shader-cache prebuild 730` or just play through it -- it resolves after the first session.
...
forge-proton-fix 252490  # Auto-diagnoses and suggests fixes
...
forge-game-profiles create 1245620   # Create a profile for Elden Ring
```

### Cyberpunk 2077

Works with Proton. HDR support via `forge-hdr`:

```bash
forge-hdr setup          # Configure HDR system-wide
forge-hdr game 1091500   # Apply HDR profile for Cyberpunk
```
...
[ProtonDB](https://www.protondb.com/) is a community database where Linux gamers report how well games work. 24HG Forge tools query ProtonDB automatically, but you can also check it manually:
...
## Using forge-proton-fix

When a game does not work, start here:

```bash
# Diagnose the issue
forge-proton-fix 12345

# Diagnose and auto-fix
forge-proton-fix 12345 --fix
```
...
Lutris manages non-Steam games (Epic, GOG, standalone). It is pre-installed on 24HG Forge.
...
Heroic manages Epic Games Store and GOG libraries. Pre-installed on 24HG Forge.
...
```bash
forge-games list        # All installed games from all launchers
forge-games search rust  # Search across all launchers
forge-games launch "Rust"  # Launch from any launcher
```

## Tips for Best Compatibility

1. **Use the latest Proton.** Newer versions fix more games. 24HG Forge auto-updates Proton-GE via `forge-proton-updater`.
2. **Check ProtonDB before buying.** If a game is rated Borked, it probably will not work.
3. **Use per-game profiles.** `forge-game-profiles` lets you set Proton version, env vars, and launch options per game.
4. **Pre-build shaders.** `forge-shader-cache prebuild <appid>` eliminates first-launch stuttering.
5. **Enable GameMode.** It is automatic on 24HG Forge via `forge-smart-launch`, but verify with `gamemoded -s`.
