...
--- Format top 5 servers from ~/.cache/24hg/server-status.json
--  Expected format: array of { "name": "...", "players": N, "max": N, "status": "online" }
function conky_server_status()
    local servers = read_json("~/.cache/24hg/server-status.json")
...
--- Format hub info from ~/.cache/24hg/hub-state.json
--  Expected format: { "username": "...", "notifications": N, "friends_online": N }
function conky_hub_status()
    local hub = read_json("~/.cache/24hg/hub-state.json")
