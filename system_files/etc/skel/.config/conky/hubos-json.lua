...
--- Format top 5 servers from ~/.cache/hubos/server-status.json
--  Expected format: array of { "name": "...", "players": N, "max": N, "status": "online" }
function conky_server_status()
    local servers = read_json("~/.cache/hubos/server-status.json")
...
--- Format hub info from ~/.cache/hubos/hub-state.json
--  Expected format: { "username": "...", "notifications": N, "friends_online": N }
function conky_hub_status()
    local hub = read_json("~/.cache/hubos/hub-state.json")
