/**
 * Forge Linux Distro - 24HG Hub API Router
 *
 * Express router providing endpoints for Forge OS installations to
 ...
 *
 * Mount at: app.use('/api/forge', forgeRouter)
 *
 * @module forge-api
 */
...
const DB_PATH = process.env.FORGE_DB_PATH || path.join(__dirname, '..', 'data', 'forge.db');
...
const GALLERY_DIR = process.env.FORGE_GALLERY_DIR || path.join(__dirname, '..', 'data', 'gallery');
...
function requireAuth(req, res, next) {
  ...
  try {
    ...
    console.error('[forge-api] Auth error:', err.message);
...
// ---------------------------------------------------------------------------
// 1. POST /heartbeat — Receive heartbeat from Forge installations
// ---------------------------------------------------------------------------

/**
 * @route POST /api/forge/heartbeat
 * @body {string} machine_id - Unique machine identifier
 * @body {string} os_version - Forge OS version string
...
router.post('/heartbeat', wrap((req, res) => {
  ...
  const stmt = db.prepare(`
    INSERT INTO forge_installations (machine_id, os_version, gpu, cpu, username, first_seen, last_seen)
    VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    ON CONFLICT(machine_id) DO UPDATE SET
      os_version = excluded.os_version,
      gpu        = COALESCE(excluded.gpu, forge_installations.gpu),
      cpu        = COALESCE(excluded.cpu, forge_installations.cpu),
      username   = COALESCE(excluded.username, forge_installations.username),
...
/**
 * @route GET /api/forge/stats
 * @returns {object} Aggregate Forge installation statistics
 */
router.get('/stats', wrap((_req, res) => {
  const total = db.prepare('SELECT COUNT(*) AS count FROM forge_installations').get().count;

  const active = db.prepare(
    "SELECT COUNT(*) AS count FROM forge_installations WHERE last_seen >= datetime('now', '-24 hours')"
  ).get().count;

  const playtime = db.prepare(
    "SELECT COALESCE(SUM(ROUND((julianday(last_seen) - julianday(first_seen)) * 24, 1)), 0) AS hours FROM forge_installations"
  ).get().hours;

  const topGpus = db.prepare(
    "SELECT gpu, COUNT(*) AS count FROM forge_installations WHERE gpu IS NOT NULL GROUP BY gpu ORDER BY count DESC LIMIT 10"
  ).all();

  const topCpus = db.prepare(
    "SELECT cpu, COUNT(*) AS count FROM forge_installations WHERE cpu IS NOT NULL GROUP BY cpu ORDER BY count DESC LIMIT 10"
  ).all();
...
/**
 * @route POST /api/forge/rig-score
...
router.post('/rig-score', requireAuth, wrap((req, res) => {
  ...
  const stmt = db.prepare(`
    INSERT INTO forge_rig_scores (machine_id, username, cpu_score, gpu_score, ram_score, storage_score, total_score, cpu_model, gpu_model, ram_gb)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
/**
 * @route GET /api/forge/rig-score/leaderboard
...
/**
 * @route POST /api/forge/voice/token
 * @auth Bearer token required
...
router.post('/voice/token', requireAuth, wrap(async (req, res) => {
  ...
  db.prepare(`
    INSERT INTO forge_voice_rooms (room_name, created_at, last_active)
    VALUES (?, datetime('now'), datetime('now'))
...
/**
 * @route GET /api/forge/voice/rooms
...
/**
 * @route POST /api/forge/achievements
 * @auth Bearer token required
 * @body {string} machine_id
 * @body {string} username
...
router.post('/achievements', requireAuth, wrap((req, res) => {
  ...
  const stmt = db.prepare(`
    INSERT INTO forge_achievements (username, achievement_id, unlocked_at, synced_at)
    VALUES (?, ?, ?, datetime('now'))
...
/**
 * @route GET /api/forge/achievements/:username
...
router.get('/achievements/:username', wrap((req, res) => {
  ...
  const achievements = db.prepare(
    'SELECT achievement_id, unlocked_at, synced_at FROM forge_achievements WHERE username = ? ORDER BY unlocked_at DESC'
  ).all(username);
...
/**
 * @route POST /api/forge/cloud-saves/sync
 * @auth Bearer token required
 * @body {string} machine_id
 * @body {string} game
...
router.post('/cloud-saves/sync', requireAuth, wrap((req, res) => {
  ...
  const existing = db.prepare(
    'SELECT id FROM forge_cloud_saves WHERE username = ? AND game = ? AND save_path = ?'
  ).get(username, game, save_path);

  if (existing) {
    db.prepare(
      "UPDATE forge_cloud_saves SET machine_id = ?, checksum = ?, size_bytes = ?, updated_at = datetime('now') WHERE id = ?"
    ).run(machine_id, checksum, size_bytes || 0, existing.id);
    return res.json({ ok: true, id: existing.id, action: 'updated' });
  }

  const info = db.prepare(
    "INSERT INTO forge_cloud_saves (username, machine_id, game, save_path, checksum, size_bytes) VALUES (?, ?, ?, ?, ?, ?)"
...
/**
 * @route GET /api/forge/cloud-saves/:username
 * @param {string} username
...
router.get('/cloud-saves/:username', wrap((req, res) => {
  ...
  const saves = db.prepare(
    'SELECT id, machine_id, game, save_path, checksum, size_bytes, updated_at FROM forge_cloud_saves WHERE username = ? ORDER BY updated_at DESC'
  ).all(username);
...
/**
 * @route POST /api/forge/game-compat
 * @auth Bearer token required
...
router.post('/game-compat', requireAuth, wrap((req, res) => {
  ...
  const info = db.prepare(`
    INSERT INTO forge_game_compat (game_name, steam_appid, works, rating, gpu, proton_version, notes, username)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
/**
 * @route GET /api/forge/game-compat/:appid
...
router.get('/game-compat/:appid', wrap((req, res) => {
  ...
  const reports = db.prepare(
    'SELECT id, game_name, works, rating, gpu, proton_version, notes, username, created_at FROM forge_game_compat WHERE steam_appid = ? ORDER BY created_at DESC'
  ).all(appid);
...
/**
 * @route GET /api/forge/vpn/peers
 * @auth Bearer token required
 * @returns {Array} Active Forge installations seen in the last 5 minutes
 */
...
/**
 * @route POST /api/forge/gallery/share
 * @auth Bearer token required
...
router.post('/gallery/share', requireAuth, upload.single('image'), wrap((req, res) => {
  ...
  const info = db.prepare(
    'INSERT INTO forge_gallery (username, filename, game, caption, file_path) VALUES (?, ?, ?, ?, ?)'
...
router.get('/gallery', wrap((req, res) => {
  ...
  let countSql = 'SELECT COUNT(*) AS count FROM forge_gallery';
  let selectSql = 'SELECT id, username, filename, game, caption, created_at FROM forge_gallery';
  const params = [];
